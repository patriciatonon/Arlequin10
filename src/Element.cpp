    ...
    for (int i = 0; i < LNN; i++)
    {
        for (int j = 0; j < LNNC; j++)
        {
            ...
            for (int k = 0; k < DIM; k++) {
                ...
                for (int l = 0; l < DIM; l++) convection +=  dphi_dx[k][i] * ((una_[l] - umeshna_[l]) * ddphiC_dx[l][k][j] + 
            ...
            for (int k = 0; k < DIM; k++){
                ...
                for (int l = 0; l < DIM; l++) {
                    ...
                    if (k == l) for (int m = 0; m < DIM; m++) viscosity += ddphiC_dx[m][m][j];
    ...
    for (int i = 0; i < LNN; i++)
    {
        for (int j = 0; j < LNN; j++)
        {
            ...
            for (int k = 0; k < DIM; k++){
                ...
                arlequinStab1[DIM*i+k][DIM*LNN+j] += 0*press * WJ;
                                                           ...
                                                           double **arlequinStab0, double *arlequinStabVector0)
{
    ...
    for (int i = 0; i < DIM; i++)
        for (int j = 0; j < DIM; j++) daccelm_dx[i][j] = alpha_m * daccel_dx[i][j] + (1. - alpha_m) * daccelPrev_dx[i][j];
    ...
    for (int i = 0; i < LNN; i++)
    {
        for (int j = 0; j < LNNC; j++)
        {
            ...
            for (int k = 0; k < DIM; k++){
                ...
                for (int l = 0; l < DIM; l++) 
                    convec +=  -dphi_dx[k][i] * ((una_[l] - umeshna_[l]) * ddphiC_dx[l][k][j] + 
                                                (duna_dx[l][k] - dumeshna_dx[k][l]) * dphiC_dx[l][j]) * tARLQ_;
            }; 
            ...
            for (int k = 0; k < DIM; k++){
                ...
                arlequinStab0[DIM*i+k][DIM*LNNC+j] += 0*press * WJ;
...
void Element<3>::computeDragAndLiftForces_FEM(int &side, double &pDForce, double &pLForce, double &fDForce, double &fLForce,
	...
	double **dphiB_;
	dphiB_ = new double*[2];
	for (int i = 0; i < 2; ++i) dphiB_[i] = new double[6];

	double **dphi_dx;
	dphi_dx = new double*[3];
	for (int i = 0; i < 3; ++i) dphi_dx[i] = new double[10];

	double **ainv_;
	ainv_ = new double*[3];
	for (int i = 0; i < 3; ++i) ainv_[i] = new double[3];

	double **Jac;
	Jac = new double*[3];
	for (int i = 0; i < 3; ++i) Jac[i] = new double[3];
	...
	for (int i = 0; i < 3; ++i) delete [] dphi_dx[i];
	delete [] dphi_dx;
	for (int i = 0; i < 2; ++i) delete [] dphiB_[i];
	delete [] dphiB_;   
	for (int i = 0; i < 3; ++i) delete [] ainv_[i];
	delete [] ainv_;
	for (int i = 0; i < 3; ++i) delete [] Jac[i];
	delete [] Jac;
